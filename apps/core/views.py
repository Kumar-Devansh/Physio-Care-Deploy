...
def prescription_pdf(request, pk):
    ...
    doctor = getattr(request.user, "doctor_profile", None)
    if doctor:
        if item.doctor_id != doctor.pk:
            raise Http404("Prescription not found")
    elif item.patient_id != _patient(request.user).pk:
        raise Http404("Prescription not found")
...
@login_required
def patient_care_summary_pdf(request, pk=None):
    doctor = getattr(request.user, "doctor_profile", None)
    if doctor:
        ...
        assignments = patient.exercise_assignments.filter(assigned_by=doctor)
        appointments = patient.appointments.filter(doctor=doctor)
    else:
        patient = _patient(request.user)
        prescriptions = patient.prescriptions.all()
        plans = patient.treatment_plans.all()
        assignments = patient.exercise_assignments.all()
        appointments = patient.appointments.all()
    lines = [
        f"Patient: {patient}",
        f"Patient ID: {patient.patient_id}",
        f"Condition: {patient.conditions or 'Not recorded'}",
        "",
        "Prescriptions:",
    ]
    lines += [
        f"- {item.issued_on:%d %b %Y}: {item.diagnosis} | {item.medicines}"
        for item in prescriptions[:10]
    ] or ["- None recorded"]
    lines += ["", "Treatment plans:"]
    lines += [
        f"- {item.title}: {item.goal} ({item.progress}% progress)"
        for item in plans[:10]
    ] or ["- None recorded"]
    lines += ["", "Assigned exercises:"]
    lines += [
        f"- {item.exercise.title}: {item.repetitions}, {item.frequency}"
        for item in assignments.select_related("exercise")[:15]
    ] or ["- None recorded"]
    lines += ["", "Recent appointments:"]
    lines += [
        f"- {item.scheduled_at:%d %b %Y}: {item.concern} ({item.get_status_display()})"
        for item in appointments.order_by("-scheduled_at")[:10]
    ] or ["- None recorded"]
    return _pdf_response(f"care-summary-{patient.patient_id}.pdf", "Patient Care Summary", lines)
    ...
    return render(
        ...
        "doctor/patient_detail.html",
        {
            ...
            "recent_messages": ChatMessage.objects.filter(
                Q(sender=doctor.user, recipient=patient.user)
                | Q(sender=patient.user, recipient=doctor.user)
            )[:6],
        },
    )
...
def doctor_action(request, action):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        if action == "prescription":
            ...
            item.save()
            extra_exercise_ids = request.POST.getlist("extra_exercise")
            extra_exercises = Exercise.objects.filter(pk__in=extra_exercise_ids).exclude(
                pk=item.exercise_id
            )
            ...
            exercise_count = 1 + extra_exercises.count()
            notice = (
                "New exercise programme",
                f"{doctor} added {exercise_count} exercise{'s' if exercise_count != 1 else ''} to your programme.",
                "exercise",
                "/exercises/",
            )
